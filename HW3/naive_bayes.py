...
def train_mle_estimator(train_images, train_labels):
    ...
    # Calculate mle for theta
    numerator = np.matmul(np.transpose(train_images), train_labels)
    denominator = np.sum(train_labels, axis=0)
    theta_mle = numerator / denominator

    # Calculate mle for pi
    N = len(train_images)
...
def train_map_estimator(train_images, train_labels):
    ...
    numerator = np.matmul(np.transpose(train_images), train_labels)
    denominator = np.sum(train_labels, axis=0)
    theta_map = (numerator + 2) / (denominator + 4)
    
    # Calculate map for pi
    N = len(train_images)
...
def log_likelihood(images, theta, pi):
    ...
    log_like = np.log(pi) + (np.dot(images, np.log(theta))) + np.dot(1 - images, np.log(1 - theta))
    
    bottom = np.zeros((rowNum, 1))
    for i in range(rowNum):
        x_i = images[i].reshape((images.shape[1],1))
        bottom[i] = np.log(np.sum(np.multiply(np.product(np.multiply(theta, x_i) + np.multiply(1 - theta,1 - x_i), axis=0), pi)))
    
    log_like = log_like - bottom
    return log_like
...
def accuracy(log_like, labels):
    ...
    accuracy = np.sum(predictions == targets) / N
    return accuracy
...
def image_sampler(theta, pi, num_images):
    ...
    sampled_images = np.zeros((num_images, theta.shape[0]))

    for i in range(num_images):
        random_c = np.random.choice(theta.shape[1], p=pi)
        probabilities = theta[:,random_c]
        sampled_images[i] = np.random.binomial(1, probabilities)
